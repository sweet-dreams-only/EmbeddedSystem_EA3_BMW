...
# error "The included header FrIf.h is incompatible with the FrIf_ExtSync.c version."
#endif
#if ( IF_ASRIFFR_RELEASE_VERSION != 0x04u )
# error "The included header FrIf.h is incompatible with the FrIf_ExtSync.c release version."
#endif
 ...
 *********************************************************************************************************************/
FUNC(Std_ReturnType, FRIF_CODE) FrIf_SetExtSync(FRIF_VCTRL_ONLY, 
                                               ...
                                               Fr_RateCorrectionType    FrIf_Rate)
{
  ...
  FrIf_CheckDetErrorReturnValue(FRIF_IS_INITIALIZED(), FRIF_SETEXTSYNC_SERVICE_ID, FRIF_E_NOT_INITIALIZED, E_NOT_OK);
  /* PRQA S 3201, 3325 1 */ /*  MD_MSR_14.1  */
  FrIf_CheckDetErrorReturnValue((0 == FRIF_VCTRL_IDX), FRIF_SETEXTSYNC_SERVICE_ID, FRIF_E_INV_CTRL_IDX, E_NOT_OK);  

  ErrorState = Fr_SetExtSync(FRIF_VCTRL_IDX, FrIf_Offset, FrIf_Rate);

  return ErrorState; 
} /* PRQA S 2006 */ /* MD_MSR_14.7 */
# endif /* if defined (FRIF_SET_EXT_SYNC_DISABLE) && (STD_OFF == FRIF_SET_EXT_SYNC_DISABLE) */
