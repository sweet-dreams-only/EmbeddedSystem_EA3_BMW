...
/**********************************************************************************************************************
  VERSION CHECK
**********************************************************************************************************************/
# define GW_ASRPDUR_FRIF_VERSION            (0x0315u)
# define GW_ASRPDUR_FRIF_RELEASE_VERSION            (0x07u)
...
/** \brief         The function is called by the FrIf to indicate the complete reception of a FrIf I-PDU.
                   The PDU Router evaluates the FrIf I-PDU handle and performs appropriate handle and port conversion.
                   The call is routed to an upper IF module using the appropriate I-PDU handle of the destination layer.
    \param[in]     FrIfRxPduId        Handle ID of the received FrIf I-PDU.
    \param[in]     PDUR_RXINDICATION_PARA
...
**********************************************************************************************************************/
/** \brief         The function is called by the FrIf to indicate the complete transmission of a FrIf I-PDU.
                   The PDU Router evaluates the FrIf I-PDU handle and performs appropriate handle and port conversion.
                   The call is routed to an upper IF module using the appropriate I-PDU handle of the destination layer.
    \param[in]     FrIfTxPduId        Handle ID of the transmitted FrIf I-PDU.
    \return        none
    \pre           PduR_Init() is executed sucessfully and the PDU Router is in PDUR_ONLINE state.
    \context       The function can be called on interrupt and task level and has not to be interrupted by other
...
**********************************************************************************************************************/
/** \brief         The function is called by the FrIf to request the FrIf TX I-PDU data before transmission.
                   The PDU Router evaluates the FrIf I-PDU handle and performs appropriate handle and port conversion.
                   The call is routed to an upper IF module using the appropriate I-PDU handle of the destination layer.
    \param[in]     FrIfTxPduId        Handle ID of the FrIf I-PDU that will be transmitted.
    \param[in,out] PDUR_TRIGGERTRANSMIT_PARA
                   SduPtr         Pointer to the I-PDU data buffer of the FrIf IF that will be transmitted.\n
                                     This Parameter is used, if PDUR_USE_PDUINFOTYPE is defined to STD_OFF.\n
                   PduInfoPtr        Payload information of the FrIf I-PDU that will be transmitted (pointer to data and data length).\n
                                     This Parameter is used, if PDUR_USE_PDUINFOTYPE is defined to STD_ON.\n
    \return        PDUR_TRIGGERTRANSMIT_RETURN
                   none                No return value.\n
                                       This return value is used, if PDUR_USE_PDUINFOTYPE is defined to STD_OFF.\n
                   Std_ReturnType      This  return value is used, if PDUR_USE_PDUINFOTYPE is defined to STD_ON.
