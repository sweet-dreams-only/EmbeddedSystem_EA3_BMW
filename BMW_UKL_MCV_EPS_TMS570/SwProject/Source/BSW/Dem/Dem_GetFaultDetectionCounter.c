 ...
 **
 ** $Id: Dem_GetFaultDetectionCounter.c 2061 2010-01-18 12:10:39Z olho2924 $
                                           ...
                                           EventIdFaultDetectionCounter)
{
   ...
   if (EventIdFaultDetectionCounter == NULL_PTR)
   {
      DEM_REPORT_ERROR(DEM_SID_GetFaultDetectionCounter, DEM_E_PARAM_ADDRESS);
      return E_NOT_OK;
   }
#endif /* DEM_DEV_ERROR_DETECT */

#if (DEM_USE_DEBOUNCE == STD_ON)
   return Dem_DebounceFctPtrTable[Dem_GbiDebounceAlgo(EventId)].
             GetFDCFct(EventId, EventIdFaultDetectionCounter);
#else
   return Dem_RteGetFaultDetectionCounter(EventId, EventIdFaultDetectionCounter);
