 ...
 ** Dem_GetWarmupCycleDTCclear().
 **
 ** \project AUTOSAR Standard Core
 ** \author Marko Schneider
 ** \author 3SOFT GmbH, 91058 Erlangen, Germany
 **
 ** $Id: Dem_GetWarmupCycleDTCclear.c 2160 2010-03-31 10:54:54Z olho2924 $
...
FUNC(Std_ReturnType, DEM_CODE) Dem_GetWarmupCycleDTCclear(
   P2VAR(Dem_DataByteType, AUTOMATIC, DEM_APPL_DATA) DataByte_A)
{
#if (DEM_DEV_ERROR_DETECT == STD_ON)
   if (Dem_InitializationState != DEM_INITIALIZED)
   {
      DEM_REPORT_ERROR(DEM_SID_GetWarmupCycleDTCclear, DEM_E_UNINIT);
      return E_NOT_OK;
   }
