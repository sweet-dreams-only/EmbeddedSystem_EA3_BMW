    ...
    )
{
    ...
    #if (STD_ON == NVM_CFG_CHK_PARAM_BLOCKID) /* Debug Mode */
        /* is the block id valid? */
        if ((NvM_NoOfBlockIds_t <= NVM_BLOCK_FROM_DCM_ID(BlockId)) || (0u == (NVM_BLOCK_FROM_DCM_ID(BlockId))))
        {
    ...
    if ((NVM_BLOCK_DATASET & NvM_BlockDescriptorTable_at[NVM_BLOCK_FROM_DCM_ID(BlockId)].MngmtType_t) == 0)
    {
        #if (STD_ON == NVM_CFG_CHK_MNGMT_TYPE) /* Debug Mode */
    ...
    )
{
    ...
    #if (STD_ON == NVM_CFG_CHK_PARAM_BLOCKID) /* Debug Mode */
        /* is the block id valid? */
        if (NvM_NoOfBlockIds_t <= NVM_BLOCK_FROM_DCM_ID(BlockId))
        {
    ...
    )
{
    ...
    /* BlockId has to be used here before BlockId check to get the pointer. But the pointer is only
      used after BlockId check. */
    const NvM_BlockDescrPtrType NvM_Descr_ptloc = &(NvM_BlockDescriptorTable_at[NVM_BLOCK_FROM_DCM_ID(BlockId)]);
    /* local ram management pointer */
    /* BlockId has to be used here before BlockId check to get the pointer. But the pointer is only
      used after BlockId check. */
    const NvM_RamMngmtPtrType NvM_RamMngmt_ptloc = 
...
FUNC(Std_ReturnType, NVM_PUBLIC_CODE) NvM_SetBlockLockStatus(NvM_BlockIdType BlockId, boolean Locked)
{
    ...
    /* BlockId has to be used here before BlockId check to get the pointer. But the pointer is only
      used after BlockId check. */
    const NvM_RamMngmtPtrType NvM_RamMngmt_ptloc = &NvM_BlockMngmtArea_at[NVM_BLOCK_FROM_DCM_ID(BlockId)];
