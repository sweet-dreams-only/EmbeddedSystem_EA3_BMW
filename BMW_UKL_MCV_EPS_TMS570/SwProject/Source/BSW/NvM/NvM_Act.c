...
STATIC FUNC(void, NVM_PRIVATE_CODE) NvM_ActInitBlock(void)
{
    ...
    {
        /* it is a DCM Block */
        orgBlockId = NVM_BLOCK_FROM_DCM_ID(NvM_CurrentJob_t.JobBlockId_t);
        DCMBlockFlag = TRUE;
    }
...
STATIC FUNC(void, NVM_PRIVATE_CODE) NvM_ActReadNvBlock(void)
{
    const uint8 crcLen = NvM_CrcJob_GetCrcLength(&NvM_CurrentBlockInfo_t.BlockCrcJob_t);
    const uint16 BlockLength = NvM_CurrentBlockInfo_t.Descriptor_pt->NvBlockLength_u16;
#if(NVM_CRC_INT_BUFFER == STD_ON)
    ...
    NvM_BlockInfoPtrType info_pt, NvM_RamAddressType destPtr, NvM_ConstRamAddressType srcPtr)
{
    ...
    const uint16 maxBytes = (NvM_NoOfCrcBytes_u16 > 32u) ? (NvM_NoOfCrcBytes_u16 >> 1) : NvM_NoOfCrcBytes_u16;
    ...
    uint16 currLength =  (info_pt->ByteCount_u16 < maxBytes) ? info_pt->ByteCount_u16 : maxBytes;
