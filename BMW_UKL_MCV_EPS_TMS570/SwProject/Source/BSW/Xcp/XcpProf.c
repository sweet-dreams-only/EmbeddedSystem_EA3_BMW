...
FUNC(void, XCP_CODE) XcpCommand( P2CONST(uint32, AUTOMATIC, XCP_APPL_DATA) pCommand )
{
 ...
 /* XCP Data Stimulation Handler */
  ...
  else /* CC_CONNECT */
  {
...
#if defined ( kXcpStationIdLength ) || defined ( XCP_ENABLE_VECTOR_MAPNAMES ) || defined ( XCP_ENABLE_VECTOR_INFOMEM )
          case CC_GET_ID:
            {
              ...
              if ( CRO_GET_ID_TYPE == IDT_ASAM_NAME )   /* Type = ASAM MC2 */
              {
                ...
                XcpSetMta( ApplXcpGetPointer(0xFF, (uint32)(&kXcpStationId[0])), 0xFF);
              }
  #endif
              ...
              if ( CRO_GET_ID_TYPE == IDT_VECTOR_INFOMEM )   /* Type = Vector Type */
              {
                ...
                XcpSetMta((MTABYTEPTR)&kInfoMem,0xFF);
              }
  #endif
            ...
            case CC_GET_DAQ_EVENT_INFO:
              {
                ...
                XcpSetMta( ApplXcpGetPointer( 0xFF, (uint32)kXcpEventName[event]), 0xFF );
...
#if defined ( XCP_ENABLE_READ_PROTECTION ) || ( defined ( XCP_ENABLE_WRITE_PROTECTION ) && defined ( XCP_ENABLE_STIM ) )
             {
                tXcpOdtIdx  odt;
                uint16      e, el;
                uint8       n, r;
                  ...
                  if(OdtEntrySize(e) > (uint8)0)
                  {
                    ...
                    while (e <= el)
                    {
                      ...
                      if (n > 0)
                      {
                          ...
                          {
                            #if defined ( XCP_ENABLE_READ_PROTECTION )
                              r = ApplXcpCheckReadAccess(OdtEntryAddr(e), (uint16)n);
                            #else
                              r = (uint8)XCP_CMD_OK;
                            #endif
                          }
                        ...
                        if ( r == (uint8)XCP_CMD_DENIED )
                        {
                          uint32 address = (uint32)OdtEntryAddr(e);
                          err = CRC_PAGE_MODE_NOT_VALID;
                          xcp.CrmLen = CRM_SET_DAQ_LIST_MODE_LEN;
                          #if defined(XCP_CPUTYPE_LITTLEENDIAN)
                            CRM_BYTE(2)= (uint8)(address      );
                            CRM_BYTE(3)= (uint8)(address >>  8);
                            CRM_BYTE(4)= (uint8)(address >> 16);
                            CRM_BYTE(5)= (uint8)(address >> 24);
                          #else
                            CRM_BYTE(2)= (uint8)(address >> 24);
                            CRM_BYTE(3)= (uint8)(address >> 16);
                            CRM_BYTE(4)= (uint8)(address >>  8);
                            CRM_BYTE(5)= (uint8)(address      );
                          #endif
                          /* Response length is arbitrary */
                          /* PRQA S 2001 1 *//* MISRA-C:2004 14.4 violation: Since efficiency is a primary implementation target it is necessary to use goto. */
                          goto negative_response1;
    ...
    #if defined ( XCP_ENABLE_PARAMETER_CHECK )
                  if ( ((uint8)CRO_WRITE_DAQ_MULIPLE_SIZE(NoDAQ)==(uint8)0u )
                     || (CRO_WRITE_DAQ_MULIPLE_SIZE(NoDAQ) > (uint8)XCP_MAX_ODT_ENTRY_SIZE)
