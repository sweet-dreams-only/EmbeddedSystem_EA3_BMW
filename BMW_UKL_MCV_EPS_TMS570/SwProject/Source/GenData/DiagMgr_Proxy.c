...
* Generation Time   : 20.11.2012 12:47:13
*****************************************************************************/
/* Version Control:
 * Date Created:      Fri Jul 13 18:34:17 2012
 * %version:          5 %
 * %date_modified:    Tue Nov 20 12:49:13 2012 %
 *---------------------------------------------------------------------------------------------------------------------
...
/* Ap_7 stateless init core function */
FUNC(void, RTE_AP_DIAGMGR_APPL_CODE) DiagMgr7_Init( void )
{
	/* Invoke stateless Init Function with reference to appropriate application */
	DiagMgr_Init_Core( 	DiagMgr_NTCInfo7_Cnt_M_str, 
						DIAGMGR_EVENTNUM_7,
						DIAGMGR_APID_7, 
						&DiagMgrInitComp7_Cnt_M_lgc,
						NTCInfoQueue7_Cnt_M_str,
						&NTCQueueIndex7_Cnt_M_u08,
						DiagSts7_Cnt_M_b16,
						ActiveRmpRate7_UlspmS_M_f32,
						&ActDiagSts7_Cnt_M_u08);
}

/* Ap_7 stateless periodic core function */
FUNC(void, RTE_AP_DIAGMGR_APPL_CODE) DiagMgr7_Per( void )
{
	/* Configurable Start of Runnable Checkpoint */
	Rte_Call_DiagMgr7_Per_CP0_CheckpointReached();
	
	/* Invoke stateless Periodic Function with reference to appropriate application */
	DiagMgr_Per_Core( 	DiagMgr_NTCInfo7_Cnt_M_str, 
						DIAGMGR_EVENTNUM_7,
						DiagSts7_Cnt_M_b16,
						ActiveRmpRate7_UlspmS_M_f32,
						&ActDiagSts7_Cnt_M_u08,
						T_NTCMapTbl7_Cnt_enum,
						&ResetNTCFlag7_Cnt_M_u08);
	
	/* Configurable End of Runnable Checkpoint */
	Rte_Call_DiagMgr7_Per_CP1_CheckpointReached();
}

/* Ap_7 stateless transition core function */
FUNC(void, RTE_AP_DIAGMGR_APPL_CODE) DiagMgr7_Trns( void )
{
	/* Invoke stateless Transition Function with reference to appropriate application */
	DiagMgr_Trns_Core( 	DiagMgr_NTCInfo7_Cnt_M_str, 
						DIAGMGR_EVENTNUM_7,
						T_NTCMapTbl7_Cnt_enum);
}

/* Ap_7 forwarding stateless proxy functions to stateless core function */
FUNC(Std_ReturnType, RTE_AP_DIAGMGR_APPL_CODE) NxtrDiagMgr7_GetNTCFailed( VAR(NTCNumber, AUTOMATIC) NTC_Cnt_T_enum, P2VAR(boolean, AUTOMATIC, AUTOMATIC) NTCFailed_Ptr_T_lgc )
{
	/* Forward request to stateless handler with reference to appropriate application data structures */
	return(NxtrDiagMgr_GetNTCFailed_Core( NTC_Cnt_T_enum, NTCFailed_Ptr_T_lgc));
}
...
/* Ap_7 forwarding stateless proxy functions to stateless core function */
FUNC(Std_ReturnType, RTE_AP_DIAGMGR_APPL_CODE) NxtrDiagMgr7_GetNTCActive( VAR(NTCNumber, AUTOMATIC) NTC_Cnt_T_enum, P2VAR(boolean, AUTOMATIC, AUTOMATIC) NTCActive_Ptr_T_lgc )
{
	/* Forward request to stateless handler with reference to appropriate application data structures */
	return(NxtrDiagMgr_GetNTCActive_Core( NTC_Cnt_T_enum, NTCActive_Ptr_T_lgc));
}
...
/* Ap_7 forwarding stateless proxy functions to stateless core function */
FUNC(Std_ReturnType, RTE_AP_DIAGMGR_APPL_CODE) NxtrDiagMgr7_GetNTCStatus( VAR(NTCNumber, AUTOMATIC) NTC_Cnt_T_enum, P2VAR(uint8, AUTOMATIC, AUTOMATIC) Status_Ptr_T_u08 )
{
	/* Forward request to stateless handler with reference to appropriate application data structures */
	return(NxtrDiagMgr_GetNTCStatus_Core( NTC_Cnt_T_enum, Status_Ptr_T_u08));
}
...
FUNC(Std_ReturnType, RTE_AP_DIAGMGR_APPL_CODE) NxtrDiagMgr7_SetNTCStatus( VAR(NTCNumber, AUTOMATIC) NTC_Cnt_T_enum, UInt8 Param_Cnt_T_u08, NxtrDiagMgrStatus Status_Cnt_T_enum )
{
	/* Forward request to stateless handler with reference to appropriate application data structures */
	return(NxtrDiagMgr_SetNTCStatus_Core( 	NTC_Cnt_T_enum,
											Param_Cnt_T_u08,
											Status_Cnt_T_enum,
											DiagMgr_NTCInfo7_Cnt_M_str,
											DiagSts7_Cnt_M_b16,
											ActiveRmpRate7_UlspmS_M_f32,
											&ActDiagSts7_Cnt_M_u08));
}
	
FUNC(Std_ReturnType, RTE_AP_DIAGMGR_APPL_CODE) NxtrDiagMgr7_ReportNTCStatus( VAR(NTCNumber, AUTOMATIC) NTC_Cnt_T_enum, UInt8 Param_Cnt_T_u08, NxtrDiagMgrStatus Status_Cnt_T_enum )
{
	VAR(Std_ReturnType, AUTOMATIC) RetVal;
	
	/* Forward request to stateless handler with reference to appropriate application data structures */
	SuspendAllInterrupts();
	if (FALSE == DiagMgrInitComp7_Cnt_M_lgc)
	{
		RetVal = NxtrDiagMgr_ReportNTCStatus_Core(	NTC_Cnt_T_enum,
													Param_Cnt_T_u08,
													Status_Cnt_T_enum,
