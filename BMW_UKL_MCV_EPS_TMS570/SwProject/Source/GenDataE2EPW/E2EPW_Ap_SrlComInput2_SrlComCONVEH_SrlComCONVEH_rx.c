 ...
 ** Module: E2EPW_Ap_SrlComInput2_SrlComCONVEH_SrlComCONVEH_rx.c
...
#include "E2EPW_CheckDeserial_Ap_SrlComInput2_SrlComCONVEH.h"
 ...
 *                communication reset.
 **********************************************************/
FUNC (void, E2EPW_CODE) E2EPW_Init_SrlComCONVEH_CON_VEH_rx (void)
{
    State_SrlComCONVEH_CON_VEH->LastValidCounter = 0;
    State_SrlComCONVEH_CON_VEH->MaxDeltaCounter  = 0;
    State_SrlComCONVEH_CON_VEH->WaitForFirstData = TRUE;
    State_SrlComCONVEH_CON_VEH->NewDataAvailable = FALSE;
    State_SrlComCONVEH_CON_VEH->LostData         = 0;
    State_SrlComCONVEH_CON_VEH->Status           = E2E_P01STATUS_NONEWDATA;
}
...
{
    return State_SrlComCONVEH_CON_VEH;
}
 ...
 * Remarks:       None
 **********************************************************/
FUNC (uint32, E2EPW_CODE) E2EPW_Read_SrlComCONVEH_CON_VEH
    (P2VAR (SG_CON_VEH, AUTOMATIC, E2EPW_APPL_DATA) AppData)
{
    /* Protected PDU area: */
    STATIC VAR (uint8, AUTOMATIC)
        ppa_SrlComCONVEH_CON_VEH_au8 [8];
    ...
    STATIC CONST (E2E_P01ConfigType, AUTOMATIC)
        ConfigVal_SrlComCONVEH_CON_VEH =
    {
          64
        , 197
        , E2E_P01_DATAID_BOTH
    ...
    STATIC P2CONST (E2E_P01ConfigType, AUTOMATIC, E2EPW_APPL_CONST)
        Config_SrlComCONVEH_CON_VEH = &ConfigVal_SrlComCONVEH_CON_VEH;

    State_SrlComCONVEH_CON_VEH->NewDataAvailable = TRUE;

    {
        ...
        if (status_u32 == status_ok_u32)
        {
            /* Return-value of Rte_Read does not affect
               following workflow.
            */
            ret_rte_u8 = Rte_Read_SrlComCONVEH_CON_VEH (AppData);
        }

        if (status_u32 == status_ok_u32)
        {
            ret_u8 = E2EPW_CheckDeserial_SrlComCONVEH (AppData);
        ...
        if (status_u32 == status_ok_u32)
        {
            uint16 crcoffset_u16 = Config_SrlComCONVEH_CON_VEH->CRCOffset/8;
            uint16 cntoffset_u16 = Config_SrlComCONVEH_CON_VEH->CounterOffset/8;
            E2EPW_Marshal_SrlComCONVEH ( AppData, ppa_SrlComCONVEH_CON_VEH_au8 );
/**********************************************************
 ...
 **********************************************************/
/* PRQA S 505 1 */
            ppa_SrlComCONVEH_CON_VEH_au8[crcoffset_u16]  = AppData->CRC_CON_VEH;
            ppa_SrlComCONVEH_CON_VEH_au8[cntoffset_u16] &= (uint8)0xf0;
            ppa_SrlComCONVEH_CON_VEH_au8[cntoffset_u16] |= AppData->ALIV_CON_VEH;
        }
    ...
    {
        ret_u8 = E2E_P01Check
                     ( Config_SrlComCONVEH_CON_VEH
                     , State_SrlComCONVEH_CON_VEH
                     , ppa_SrlComCONVEH_CON_VEH_au8
                     );
        status_u32 &= 0x80FFFF00U;
        status_u32 |= ret_u8;
        status_u32 |= (uint32)(State_SrlComCONVEH_CON_VEH->Status<<24);
    }
