...
* Generation Time   : 07.06.2013 20:26:04
*****************************************************************************/
/* Version Control:
 * Date Created:      Fri Jul 13 18:34:17 2012
 * %version:          1 %
 * %date_modified:    Thu Mar  6 14:38:50 2014 %
 *---------------------------------------------------------------------------------------------------------------------
...
/* Ap_7 periodic function */
FUNC(void, RTE_AP_STAMD_APPL_CODE) StaMd7_Per1( void )
{

	/* Configurable Start of Runnable Checkpoint */
	Rte_Call_StaMd7_Per1_CP0_CheckpointReached();
	
	SystemState7_Cnt_M_enum = Rte_Mode_SystemState7_Mode();
	
	if ((SystemState_Cnt_M_enum != SystemState7_Cnt_M_enum) && (SystemState7_Cnt_M_enum != RTE_TRANSITION_StaMd_Mode))
	{
		Rte_Switch_Ap_StaMd7_SystemState7_Mode(SystemState_Cnt_M_enum);
	}
	
	/* Configurable End of Runnable Checkpoint */
	Rte_Call_StaMd7_Per1_CP1_CheckpointReached();
}

FUNC(void, RTE_AP_STAMD_APPL_CODE) MilestoneRqst7_WarmInitMilestoneComplete(StaMd_Users user)
{
	VAR(uint32, AUTOMATIC) MilestoneRqst_Cnt_T_u32;

	Rte_Enter_StaMds7_MilestoneRqst_WARMINIT_ExclArea();
	
	MilestoneRqst_Cnt_T_u32 = WarmInitMilestoneRqst7_Cnt_M_u32;
	ClrBits_m(MilestoneRqst_Cnt_T_u32, (~(1U << user)));
	WarmInitMilestoneRqst7_Cnt_M_u32 = MilestoneRqst_Cnt_T_u32;
	
	Rte_Exit_StaMds7_MilestoneRqst_WARMINIT_ExclArea();
}

FUNC(void, RTE_AP_STAMD_APPL_CODE) MilestoneRqst7_WarmInitMilestoneNotComplete(StaMd_Users user)
{
	VAR(uint32, AUTOMATIC) MilestoneRqst_Cnt_T_u32;
