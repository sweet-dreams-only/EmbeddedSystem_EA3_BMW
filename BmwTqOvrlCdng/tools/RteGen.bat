...
attrib -r "..\tools\contract\*.h" >>logfile.txt
 ...
 set dcf=Ap_BmwTqOvrlCdng


set SWC=Ap_BmwTqOvrlCdng


..\..\Autosar3Tools\RteGen\dvrtegen -d "..\autosar\%dcf%.dcf" -m %SWC% -g i -o "..\src" >>logfile.txt
..\..\Autosar3Tools\RteGen\dvrtegen -d "..\autosar\%dcf%.dcf" -m %SWC% -g c -o "..\tools\contract\Ap_BmwTqOvrlCdng" >>logfile.txt
pause