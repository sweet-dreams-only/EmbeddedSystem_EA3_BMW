...
* 08/15/16   5       AJM       Added configuration for SCI Data transfer                                       EA3#9825
* 08/30/16   6       AJM       Changed Dma_Init to Dma_Init1 for compatibility with Vector SPI driver          EA3#10590
***********************************************************************************************************************/
...
/******************************************************************************
  ...
  ****************************************************************************/
FUNC(void, DMA_CODE) Dma_Init1(void)
{
	...
	else
	{
		...
		/* If DMA used only for FlsTst, use most restrictive MPU settings */
			...
			(D_SCIFORIMCGROUPENABLED_CNT_ENUM == STD_OFF)
		...
		#if (D_SCIFORIMCGROUPENABLED_CNT_ENUM == STD_OFF)
			DMACTRLREG->DMAMPR0E = 0xFF473FFFul;
		#else
			DMACTRLREG->DMAMPR0E = 0xFFF7E47Ful;
		#endif
...
/******************************************************************************
  * Name:        Dma_SetupSciTrfGroup
  * Description: Performs DMA register settings needed to set up DMA Channels 14
				 & 15 for SCI Tx and RX respectively
  *              FlsTst Flash CRC Test
  * Inputs:      NONE  
  * Outputs:     NONE 
  * Usage Notes:  Called by the SCI Init function.  
  ****************************************************************************/
FUNC(void, DMA_CODE) Dma_SetupSciTrfGroup(void)
{
#if (D_SCIFORIMCGROUPENABLED_CNT_ENUM == STD_ON)
	
	/* DMA Channel Configuration for Transmission */
    DMACTRLPKT14->ISADDR = (uint32)(&DMAData_G_str.SciTxBuf_Cnt_u32[0U]);  /* Source Address */
    DMACTRLPKT14->IDADDR = 0xFFF7E474U;       /* Destination Address */
    
    DMACTRLPKT14->CHCTRL =  (2UL << 14) |    	/* Read Size - 32 Bits*/
                           (2UL << 12) |     	/* Write Size - 32 Bits */
                           (3UL << 3)  |     	/* Indexed Addressing mode for Read */
                           (3UL << 1)       	/* Indexed Addressing mode for Write */
                           | (1UL);         	/* Auto Initiation Mode enabled */
                           
    DMACTRLPKT14->EIOFF = (4UL << 16) |      	/* Element Index Offset for Destination */
                         (4UL);             	/* Element Index Offset for Source */
                           
    DMACTRLPKT14->FIOFF = 8UL ;              	/* Element Index Offset for Destination is 0 , Element Index Offset for source is 8 */

	DMACTRLREG->PAR1 |= (4ul << 4);	/* Channel 14:	Port B */

    
    /* Channel configuration for SCI Reception */
    DMACTRLPKT15->ISADDR = 0xFFF7E464U;           			/* Source Address */
    DMACTRLPKT15->IDADDR = (uint32)(&DMAData_G_str.SciRxBuf_Cnt_u32[0U]);      /* Destination Address */
    DMACTRLPKT15->ITCOUNT = ((D_MAXSIGGROUPFORTX_CNT_U08 * 2UL) << 16U) |    				/* Frame Count */
							2U;               				/* Element Count */    
    
    DMACTRLPKT15->CHCTRL = (2UL << 14U) |    				/* Read Size - 32 Bits*/
                           (2UL << 12U) |    				/* Write Size - 32 Bits */
                           (3UL << 3U)  |    				/* Indexed Addressing mode for Read */
                           (3UL << 1U)  |    				/* Indexed Addressing mode for Write */ 
                           (1UL);           				/* Auto Initiation Enabled */
                           
    DMACTRLPKT15->EIOFF = (4UL << 16U) |      				/* Element Index Offset for Destination */
                          (4UL);             				/* Element Index Offset for Destination */

    DMACTRLPKT15->FIOFF = (8UL << 16) ;      				/* Frame Index Offset for Destination is 8 , Frame Index Offset for source is 0 */
	
	DMACTRLREG->PAR1 |= (4ul);	/* Channel 15:	Port B */	
#endif
}

/******************************************************************************
  * Name:        Dma_SciStartTx
  * Description: Enables the DMA channel to transfer data from Tx buffer to 
				 SCI buffer
  * Inputs:      Transfer Count     
  * Outputs:     NONE 
  * Usage Notes:  Called by the SCI TX function 
  ****************************************************************************/
FUNC(void, DMA_CODE) Dma_SciStartTx(void)
{
#if (D_SCIFORIMCGROUPENABLED_CNT_ENUM == STD_ON)
	
	/* Enable Channel */
	DMACTRLREG->SWCHENAS = 1UL << 14U;        /* Trigger a Transmit Request from DMA */
	
#endif
}
/******************************************************************************
  * Name:        Dma_SciStartRx
  * Description: Enables the DMA channel to transfer data from SCI Rx buffer to 
				 result buffer
  * Inputs:      NONE     
  * Outputs:     NONE 
  * Usage Notes:  Called by the SCI RX function 
  ****************************************************************************/
FUNC(void, DMA_CODE) Dma_SciStartRx(void)
{
#if (D_SCIFORIMCGROUPENABLED_CNT_ENUM == STD_ON)
	
	/* Enable Channel */
	DMACTRLREG->SWCHENAS = 1UL << 15U;		 /* Trigger a Receive Request for DMA */
	
#endif
}

/******************************************************************************
  * Name:        Dma_SciUpdFrmTxCnt
  * Description: Updates the Tx count value to ITCOUNT DMA register
  * Inputs:      Transfer Count     
  * Outputs:     NONE 
  * Usage Notes:  Called by the SCI TX function 
  ****************************************************************************/
FUNC(void, DMA_CODE) Dma_SciUpdFrmTxCnt(uint8 TrfCnt_Cnt_T_u08)
{
#if (D_SCIFORIMCGROUPENABLED_CNT_ENUM == STD_ON)
	   	DMACTRLPKT14->ITCOUNT =	((uint32)TrfCnt_Cnt_T_u08 << 16)	|	/* Frame Count */
   							2U;				/* Element Count */		
#endif
}
...
/******************************************************************************
  * Name:        Dma_SciRxPnd
  * Description: Updates the Sci Rx Pending Status
  * Inputs:      NONE      
  * Outputs:     Receive Status 
  * Usage Notes:  Called by the SCI 2ms RX function 
  ****************************************************************************/
FUNC(boolean, DMA_CODE) Dma_SciRxPnd(void)
{
	boolean DmaRxPndSts_Cnt_T_lgc = FALSE;
#if (D_SCIFORIMCGROUPENABLED_CNT_ENUM == STD_ON)

	if((DMACTRLREG->PEND & (1UL<<15)) != 0U)
	{
		DmaRxPndSts_Cnt_T_lgc = TRUE;				
	}
#endif
	return	DmaRxPndSts_Cnt_T_lgc;
}
